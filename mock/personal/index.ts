...
const timeLineType = ['info', 'success', 'warning', 'error']
...
const createTimeLineList = () => {
  return timeLineText.map(c => {
    return c.split('').map(text => {
      return {
        type: Random.pick(timeLineType),
        text
      }
    })
  })
...
const createPersonalUser = () => {
  return {
    ...
    timeLine: Random.pick(createTimeLineList())
  }
}
