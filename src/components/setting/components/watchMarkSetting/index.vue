<template>
  <section>
    ...
    <n-grid flex justify-between items-center mb-12px>
      <n-gi :span="8">开启类型</n-gi>
      <n-gi :span="16">
        <n-select
          size="small"
          :disabled="!waterMarkActive"
          v-model:value="waterMarkType"
        ...
        <n-tooltip v-else trigger="hover">
          <template #trigger>
            <n-input
              size="small"
              :disabled="!waterMarkActive"
              v-model:value="waterMarkImageUrl"
              placeholder="图片地址"
            />
          </template>
          记得注意图像 URL 的跨域设置
        ...
        <n-tooltip v-else trigger="hover">
          <template #trigger>
            <n-input-number
              ...
              size="small"
              :min="1"
              v-model:value="waterMarkImageSize"
              placeholder="请输入"
            />
          </template>
          修改图片大小后，请重新开启水印
    ...
    <n-grid flex justify-between items-center mt-12px>
      <n-gi :span="8">旋转角度</n-gi>
      <n-gi :span="16">
        <n-input-number
          v-if="waterMarkType === 'text'"
          :disabled="!waterMarkActive"
          size="small"
          v-model:value="waterMarkRotate"
          placeholder="...."
        />
        <n-input-number
          v-else
          size="small"
          :disabled="!waterMarkActive"
          v-model:value="watchMarkImageRotate"
          placeholder="...."
        />
      </n-gi>
    </n-grid>
